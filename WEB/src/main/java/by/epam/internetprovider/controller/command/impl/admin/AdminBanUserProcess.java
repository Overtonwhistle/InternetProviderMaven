...
public class AdminBanUserProcess implements ICommand {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		int banReason = Integer.parseInt(request.getParameter(PARAMETER_BAN_REASON));

		try {
			internetProviderService.banUser(userId, banReason,
					request.getParameter(PARAMETER_BAN_COMMENT));
		} catch (ServiceException e) {
			logger.log(Level.ERROR, "Failed to edit user data in command::AdminBanUserProcess");
			throw new CommandException("Error executing command::AdminBanUserProcess", e);
		}
